...
void LETTR(int16_t w,int16_t h,int16_t x,int16_t y,const unsigned long *_digit_arry_snt_);
...
void LETTR(int16_t w,int16_t h,int16_t x1,int16_t y1,const unsigned long *_digit_arry_snt_)
{
	...
	for (int16_t j = 0; j < h; j++, y++) {
		for (int16_t i = 0; i < w; i++) {
			...
			if (byte & 0x80)
			{
				Draw_Pixel(x + i, y, TXT_COLOR);
			}
			else
			{
				Draw_Pixel(x + i, y, background_color);
			}
